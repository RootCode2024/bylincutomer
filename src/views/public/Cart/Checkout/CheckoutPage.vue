<template>
  <div class="max-w-6xl mx-auto p-4 sm:p-5 mt-8 sm:mt-12 mb-8 sm:mb-12">
    <!-- Progress Bar -->
    <div class="mb-8 sm:mb-10">
      <div class="flex justify-between relative">
        ...
        <div 
          ...
          :key="step.id" 
          class="flex flex-col items-center relative z-20 cursor-pointer flex-1 min-w-0"
          @click="navigateToStep(index)"
        >
          <div 
            class="w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center font-semibold mb-2 transition-all duration-300 border-2 text-xs sm:text-base"
            :class="{
          ...
          >
            <svg v-if="currentStep > index" class="w-3 h-3 sm:w-5 sm:h-5" fill="currentColor" viewBox="0 0 20 20">
              <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/>
            </svg>
            <span v-else>{{ index + 1 }}</span>
          </div>
          <div 
            class="text-xs sm:text-sm font-medium transition-colors duration-300 text-center px-1"
            :class="{
    ...
    <div v-if="loading" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white p-4 sm:p-6 rounded-lg flex items-center shadow-lg mx-4">
        <svg class="animate-spin h-4 w-4 sm:h-5 sm:w-5 text-blue-600 mr-3" viewBox="0 0 24 24">
          <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" fill="none"></circle>
          <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
        </svg>
        <span class="text-sm sm:text-base">Traitement en cours...</span>
      </div>
    </div>

    <!-- Error Notification -->
    <div v-if="errorMessage" class="fixed top-4 right-4 left-4 sm:left-auto bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg shadow-lg z-50 max-w-md mx-auto sm:mx-0">
      <div class="flex justify-between items-start">
        <span class="text-sm">{{ errorMessage }}</span>
        <button @click="errorMessage = ''" class="ml-4 text-red-800 hover:text-red-900 text-lg leading-none flex-shrink-0">
          &times;
        </button>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
      ...
      <div class="lg:col-span-2">
        <div class="bg-white rounded-xl p-4 sm:p-6 lg:p-8 shadow-sm border border-gray-100">
          ...
          <section v-show="currentStep === 0" class="space-y-6">
            <div class="flex items-center gap-3 mb-6">
              ...
              <h2 class="text-xl sm:text-2xl font-semibold text-gray-900">Adresse de livraison</h2>
            </div>
            ...
            <div v-if="normalizedUserAddresses && normalizedUserAddresses.length > 0" class="mb-6">
              ...
              <select
                ...
                class="w-full px-3 sm:px-4 py-2 sm:py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition text-sm bg-white"
              >
            ...
            <div v-if="selectedAddress === 'new' || !normalizedUserAddresses || normalizedUserAddresses.length === 0" 
                class="space-y-6 p-4 sm:p-6 bg-gray-50 rounded-lg border-2 border-dashed border-gray-200">
              
              <div class="text-xs sm:text-sm text-gray-600 mb-4">
                <svg class="w-3 h-3 sm:w-4 sm:h-4 inline mr-2" fill="currentColor" viewBox="0 0 20 20">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                <div>
                  ...
                  <select 
                    ...
                    class="w-full px-3 py-2 sm:py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition bg-white text-sm sm:text-base"
                    required
                  >
                ...
                <div class="md:col-span-2">
                  ...
                  <input 
                    ...
                    class="w-full px-3 py-2 sm:py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition text-sm sm:text-base"
          ...
          <section v-show="currentStep === 1" class="space-y-6">
            <div class="flex items-center gap-3 mb-6">
              ...
              <h2 class="text-xl sm:text-2xl font-semibold text-gray-900">Méthode de livraison</h2>
            </div>
            ...
            <div class="space-y-4">
              ...
              <template v-if="shipping.country_code === 'BJ'">
                ...
                <div 
                  class="relative p-3 sm:p-4 border-2 rounded-xl cursor-pointer transition-all duration-200 hover:border-blue-300"
                  :class="{ 
                ...
                >
                  <div class="flex items-center">
                    ...
                    <div class="flex-1 ml-3 sm:ml-4">
                      <div class="flex items-center gap-2 font-medium text-gray-900 text-sm sm:text-base">
                      ...
                      <div class="text-xs sm:text-sm text-gray-600 mt-1">{{ getShippingMethod('local_standard')?.details }}</div>
                    </div>
                    <div class="text-sm sm:text-lg font-semibold text-green-600 whitespace-nowrap ml-2">A LA LIVRAISON</div>
                  </div>
                  <div class="mt-2 sm:mt-3 p-2 sm:p-3 bg-green-50 text-green-800 text-xs sm:text-sm rounded-lg">
                ...
                >
                  <div class="flex items-center">
                    ...
                    <div class="flex-1 ml-3 sm:ml-4">
                      ...
                      <div class="text-xs sm:text-sm text-gray-600 mt-1">{{ getShippingMethod('local_express')?.details }}</div>
                    </div>
                    <div class="text-sm sm:text-lg font-semibold text-blue-600 whitespace-nowrap ml-2">{{ formatCurrency(getShippingMethod('local_express')?.price) }}</div>
                  </div>
                </div>
              </template>
                ...
                >
                  <div class="flex items-center">
                    ...
                    <div class="flex-1 ml-3 sm:ml-4">
                      ...
                      <div class="text-xs sm:text-sm text-gray-600 mt-1">{{ getShippingMethod('dhl_express')?.details }}</div>
                    </div>
                    <div class="text-sm sm:text-lg font-semibold text-red-600 whitespace-nowrap ml-2">{{ formatCurrency(getShippingMethod('dhl_express')?.price) }}</div>
                  </div>
                </div>
              </template>
            </div>
          ...
          <section v-show="currentStep === 2" class="space-y-6">
            <div class="flex items-center gap-3 mb-6">
              <div class="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl flex items-center justify-center shadow-md">
                <svg class="w-4 h-4 sm:w-5 sm:h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/>
                </svg>
              </div>
              <h2 class="text-xl sm:text-2xl font-bold text-gray-900">Paiement Mobile Money</h2>
            </div>

            <!-- Mobile Money Options -->
            <div class="space-y-6 p-4 sm:p-6 bg-gradient-to-br from-white to-gray-50 border-2 border-gray-200 rounded-2xl shadow-sm">
              <div class="text-center mb-6">
                <h3 class="text-base sm:text-lg font-bold text-gray-900 mb-2">Choisissez votre opérateur mobile</h3>
                <p class="text-xs sm:text-sm text-gray-600">Paiement sécurisé via Mobile Money</p>
              </div>
              ...
              <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
                ...
                <div 
                  ...
                  class="group p-3 sm:p-4 border-3 rounded-xl sm:rounded-2xl text-center cursor-pointer transition-all duration-300 hover:shadow-lg sm:hover:shadow-xl hover:-translate-y-1"
                  :class="{ 
                    'border-yellow-500 bg-gradient-to-br from-yellow-50 to-yellow-100 shadow-md sm:shadow-lg scale-105': selectedMobileMoney === 'mtn', 
                ...
                >
                  <div class="mb-3 sm:mb-4 flex justify-center">
                    <div style="background-color: #ffcb05;" class="w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 rounded-xl sm:rounded-2xl shadow-md flex items-center justify-center p-2 sm:p-3 transition-transform group-hover:scale-105">
                      <img 
                  ...
                  <div class="text-sm sm:text-base font-bold text-gray-900">MTN Mobile Money</div>
                  <div class="text-xs text-gray-600 mt-1">Paiement instantané</div>
                  <div v-if="selectedMobileMoney === 'mtn'" class="mt-2 sm:mt-3 flex items-center justify-center">
                ...
                <div 
                  ...
                  class="group p-3 sm:p-4 border-3 rounded-xl sm:rounded-2xl text-center cursor-pointer transition-all duration-300 hover:shadow-lg sm:hover:shadow-xl hover:-translate-y-1"
                  :class="{ 
                    'border-blue-500 bg-gradient-to-br from-blue-50 to-blue-100 shadow-md sm:shadow-lg scale-105': selectedMobileMoney === 'moov', 
                    'border-gray-200 bg-white hover:border-blue-300': selectedMobileMoney !== 'moov' 
                  }"
                  @click="selectMobileMoney('moov')"
                >
                  <div class="mb-3 sm:mb-4 flex justify-center">
                    <div style="background-color: #0055bb;" class="w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 rounded-xl sm:rounded-2xl shadow-md flex items-center justify-center p-2 sm:p-3 transition-transform group-hover:scale-105">
                      <img 
                  ...
                  <div class="text-sm sm:text-base font-bold text-gray-900">Moov Money</div>
                  <div class="text-xs text-gray-600 mt-1">Paiement instantané</div>
                  <div v-if="selectedMobileMoney === 'moov'" class="mt-2 sm:mt-3 flex items-center justify-center">
                    <svg class="w-4 h-4 sm:w-5 sm:h-5 text-blue-600" fill="currentColor" viewBox="0 0 20 20">
                      <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                    </svg>
                  </div>
                ...
                <div 
                  ...
                  class="group p-3 sm:p-4 border-3 rounded-xl sm:rounded-2xl text-center cursor-pointer transition-all duration-300 hover:shadow-lg sm:hover:shadow-xl hover:-translate-y-1"
                  :class="{ 
                    'border-blue-500 bg-gradient-to-br from-blue-50 to-blue-100 shadow-md sm:shadow-lg scale-105': selectedMobileMoney === 'celtiis', 
                    'border-gray-200 bg-white hover:border-blue-300': selectedMobileMoney !== 'celtiis' 
                  }"
                  @click="selectMobileMoney('celtiis')"
                >
                  <div class="mb-3 sm:mb-4 flex justify-center">
                    <div style="background-color: #1c3b72;" class="w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 rounded-xl sm:rounded-2xl shadow-md flex items-center justify-center p-2 sm:p-3 transition-transform group-hover:scale-105">
                      <img 
                  ...
                  <div class="text-sm sm:text-base font-bold text-gray-900">Celtiis Cash</div>
                  <div class="text-xs text-gray-600 mt-1">Paiement instantané</div>
                  <div v-if="selectedMobileMoney === 'celtiis'" class="mt-2 sm:mt-3 flex items-center justify-center">
                    <svg class="w-4 h-4 sm:w-5 sm:h-5 text-blue-600" fill="currentColor" viewBox="0 0 20 20">
                      <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                    </svg>
                  </div>
                ...
                <div 
                  ...
                  class="group p-3 sm:p-4 border-3 rounded-xl sm:rounded-2xl text-center cursor-pointer transition-all duration-300 hover:shadow-lg sm:hover:shadow-xl hover:-translate-y-1"
                  :class="{ 
                    'border-orange-500 bg-gradient-to-br from-orange-50 to-orange-100 shadow-md sm:shadow-lg scale-105': selectedMobileMoney === 'orange', 
                    'border-gray-200 bg-white hover:border-orange-300': selectedMobileMoney !== 'orange' 
                  }"
                  @click="selectMobileMoney('orange')"
                >
                  <div class="mb-3 sm:mb-4 flex justify-center">
                    <div class="w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 bg-white rounded-xl sm:rounded-2xl shadow-md flex items-center justify-center p-2 sm:p-3 transition-transform group-hover:scale-105">
                      <img 
                  ...
                  <div class="text-sm sm:text-base font-bold text-gray-900">Orange Money</div>
                  <div class="text-xs text-gray-600 mt-1">Paiement instantané</div>
                  <div v-if="selectedMobileMoney === 'orange'" class="mt-2 sm:mt-3 flex items-center justify-center">
                    <svg class="w-4 h-4 sm:w-5 sm:h-5 text-orange-600" fill="currentColor" viewBox="0 0 20 20">
                      <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                    </svg>
                  </div>
                </div>
                ...
                <div 
                  ...
                  class="group p-3 sm:p-4 border-3 rounded-xl sm:rounded-2xl text-center cursor-pointer transition-all duration-300 hover:shadow-lg sm:hover:shadow-xl hover:-translate-y-1"
                  :class="{ 
                    'border-[#1AB3E5] bg-gradient-to-br from-[#1ab2e53a] to-[#1ab2e593] shadow-md sm:shadow-lg scale-105': selectedMobileMoney === 'wave', 
                ...
                >
                  ...
                  <div class="text-sm sm:text-base font-bold text-gray-900">Wave</div>
                  <div class="text-xs text-gray-600 mt-1">Paiement instantané</div>
                  <div v-if="selectedMobileMoney === 'wave'" class="mt-2 sm:mt-3 flex items-center justify-center">
                    <svg class="w-4 h-4 sm:w-5 sm:h-5 text-[#1AB3E5]" fill="currentColor" viewBox="0 0 20 20">
                      <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                    </svg>
                  </div>
                </div>
                ...
                <div 
                  ...
                  class="group p-3 sm:p-4 border-3 rounded-xl sm:rounded-2xl text-center cursor-pointer transition-all duration-300 hover:shadow-lg sm:hover:shadow-xl hover:-translate-y-1"
                  :class="{ 
                    'border-[#FFD100] bg-gradient-to-br from-[#ffd00056] to-[#ffd0006f] shadow-md sm:shadow-lg scale-105': selectedMobileMoney === 'freesn', 
                    'border-[#FFD100]-white hover:border-[#FFD100]': selectedMobileMoney !== 'freesn' 
                  }"
                  @click="selectMobileMoney('freesn')"
                >
                  <div class="mb-3 sm:mb-4 flex justify-center">
                    <div class="w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 bg-[#FFD100] rounded-xl sm:rounded-2xl shadow-md flex items-center justify-center p-2 sm:p-3 transition-transform group-hover:scale-105">
                      <img 
                  ...
                  <div class="text-sm sm:text-base font-bold text-gray-900">Free Money</div>
                  <div class="text-xs text-gray-600 mt-1">Paiement instantané</div>
                  <div v-if="selectedMobileMoney === 'freesn'" class="mt-2 sm:mt-3 flex items-center justify-center">
                    <svg class="w-4 h-4 sm:w-5 sm:h-5 text-[#FFD100]" fill="currentColor" viewBox="0 0 20 20">
                      <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                    </svg>
                  </div>
                </div>
              ...
              <transition name="fade-slide">
                <div v-if="selectedMobileMoney" class="space-y-4 p-4 sm:p-6 bg-white rounded-xl border-2 border-gray-200 shadow-sm">
                  <div class="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <h4 class="font-bold text-gray-900 text-sm sm:text-base">Numéro de téléphone</h4>
                      <p class="text-xs sm:text-sm text-gray-600">Entrez votre numéro {{ selectedMobileMoney.toUpperCase() }}</p>
                    </div>
                  </div>

                  <div class="flex flex-col sm:flex-row gap-3">
                    <!-- Country Code Select -->
                    <div class="relative sm:flex-1 sm:max-w-44">
                      <select 
                        v-model="mobilePayment.countryCode"
                        class="w-full h-full pl-3 sm:pl-4 pr-8 sm:pr-10 py-3 sm:py-4 text-sm sm:text-base border-2 border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all font-medium appearance-none bg-white cursor-pointer hover:border-gray-400"
                      >
                    ...
                    <div class="sm:flex-1">
                      <input 
                        ...
                        maxlength="15"
                        class="block w-full px-3 sm:px-4 py-3 sm:py-4 text-base sm:text-lg border-2 border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all font-medium placeholder-gray-400"
                  ...
                  <div class="flex items-start gap-2 p-3 bg-blue-50 rounded-lg border border-blue-200">
                    <svg class="w-4 h-4 sm:w-5 sm:h-5 text-blue-600 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
          ...
          <!-- Confirmation -->
          <section v-show="currentStep === 3" class="text-center py-8 sm:py-12">
            <div class="max-w-md mx-auto px-4">
              ...
              <h2 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Commande confirmée !</h2>
              <p class="text-lg sm:text-xl text-blue-600 mb-4 font-semibold">N° {{ orderNumber }}</p>
              <p class="text-gray-600 mb-6 sm:mb-8 text-sm sm:text-base">Nous avons envoyé les détails à <strong>{{ shipping.email }}</strong></p>
              
              <div class="bg-gray-50 rounded-xl p-4 sm:p-6 text-left space-y-3 sm:space-y-4 mb-6 sm:mb-8 border text-sm sm:text-base">
                <div class="flex justify-between items-center">
                  <span class="text-gray-600 font-medium">Date estimée de livraison</span>
                  <span class="font-semibold text-gray-900 text-right text-xs sm:text-sm">{{ estimatedDelivery }}</span>
                </div>
                <div class="flex justify-between items-center">
                  <span class="text-gray-600 font-medium">Méthode de livraison</span>
                  <span class="font-semibold text-gray-900 text-right text-xs sm:text-sm">{{ selectedShippingMethod.name }}</span>
                </div>
                <div class="flex justify-between items-start">
                  <span class="text-gray-600 font-medium">Adresse de livraison</span>
                  <span class="font-semibold text-gray-900 text-right text-xs sm:text-sm">
              ...
              <div class="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
                <button @click="trackOrder" class="flex items-center justify-center gap-2 px-4 sm:px-6 py-2 sm:py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium text-sm sm:text-base">
                  <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path>
                  </svg>
                  Suivre ma commande
                </button>
                <button @click="continueShopping" class="flex items-center justify-center gap-2 px-4 sm:px-6 py-2 sm:py-3 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors font-medium text-sm sm:text-base">
                  <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <!-- Navigation Buttons -->
          <div class="flex justify-between items-center mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-gray-200">
            <button 
              v-if="currentStep > 0 && currentStep < 3"
              @click="previousStep"
              class="flex items-center gap-2 px-4 sm:px-6 py-2 sm:py-3 text-gray-600 hover:text-gray-800 hover:bg-gray-50 rounded-lg transition-colors font-medium text-sm sm:text-base"
            >
            ...
            <button 
              v-if="currentStep < 3"
              @click="nextStep"
              class="flex items-center gap-2 px-4 sm:px-6 py-2 sm:py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium shadow-sm text-sm sm:text-base"
              :disabled="!canProceed || loading"
              :class="{ 'opacity-50 cursor-not-allowed': !canProceed || loading }"
            >
              <span>{{ getNextButtonText() }}</span>
              <svg v-if="!loading" class="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="lg:col-span-1">
        <div class="bg-white rounded-xl p-4 sm:p-6 shadow-sm border border-gray-100 sticky top-4">
          <div class="flex items-center gap-2 mb-4 sm:mb-6">
            <svg class="w-4 h-4 sm:w-5 sm:h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/>
            </svg>
            <h3 class="text-base sm:text-lg font-semibold text-gray-900">Récapitulatif</h3>
          </div>
          
          <!-- Items List -->
          <div class="space-y-3 sm:space-y-4 mb-4 sm:mb-6 max-h-60 overflow-y-auto">
            <div v-for="item in cartItems" :key="item.id" 
                 class="flex justify-between items-start text-xs sm:text-sm bg-gray-50 p-2 sm:p-3 rounded-lg">
              <div class="flex-1">
                <div class="font-medium text-gray-900 mb-1">{{ item.name }}</div>
                <div class="text-gray-600">Qté: {{ item.quantity }}</div>
              </div>
              <div class="font-medium text-gray-900 whitespace-nowrap ml-2">{{ formatCurrency(item.price * item.quantity) }}</div>
            </div>
          </div>
          
          <!-- Summary Totals -->
          <div class="border-t border-gray-200 pt-3 sm:pt-4 space-y-2 sm:space-y-3 text-xs sm:text-sm">
            <div class="flex justify-between text-gray-600">
              <span>Sous-total ({{ cartItems.length }} article{{ cartItems.length > 1 ? 's' : '' }})</span>
              <span>{{ formatCurrency(subtotal) }}</span>
            </div>
            
            <div v-if="cartStore.couponValue && cartStore.couponValue > 0" class="flex justify-between text-red-600">
              <span>Reduction</span>
              <span>-{{ formatCurrency(cartStore.couponValue) }}</span>
            </div>
            
            <div v-if="shippingCost > 0" class="flex justify-between text-gray-600">
              <span>Livraison</span>
              <span>{{ formatCurrency(shippingCost) }}</span>
            </div>
            
            <div v-else-if="shipping.method === 'local_standard'" class="flex justify-between text-green-600">
              <span>Livraison</span>
              <span class="font-medium text-xs sm:text-sm">A LA LIVRAISON</span>
            </div>
            
            <div class="flex justify-between font-bold text-base sm:text-lg border-t border-gray-200 pt-2 sm:pt-3 text-gray-900">
              <span>Total</span>
          ...
          <!-- Security Notice -->
          <div class="mt-4 sm:mt-6 p-2 sm:p-3 bg-green-50 rounded-lg border border-green-200">
            <div class="flex items-center gap-2 text-green-800 text-xs sm:text-sm">
...
<!-- Le script reste EXACTEMENT le même, je ne le modifie pas -->
<script setup>
