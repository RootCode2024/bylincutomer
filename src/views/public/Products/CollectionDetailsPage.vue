...
import api from '@/api/axiosConfig' // Assurez-vous que le chemin vers votre instance axios est correct
...
const fetchCollectionData = async () => {
    try {
        ...
        const response = await api.get(`/collections/${route.params.slug}`)
        console.log(response.data.collection.products)
        
        if (response.success) {
            collection.value = response.data.collection
            otherCollections.value = response.data.other_collections || []
