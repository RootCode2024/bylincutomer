...
const loadRelatedTutorials = async () => {
  try {
    if (!tutorial.value || !tutorial.value.categories.length) return
    
    const categorySlug = tutorial.value.categories[0].slug
    const response = await api.get(`/tutorials/category/${categorySlug}`)
    
    if (response.success) {
      relatedTutorials.value = response.data
        .filter(t => t.id !== tutorial.value.id)
        .slice(0, 3)
    }
  } catch (err) {
    console.error('Erreur lors du chargement des tutoriels similaires:', err)
  }
}
...
const loadPurchasedTutorials = async () => {
  try {
    ...
    const response = await api.get('/user/purchased-tutorials', {
      headers: {
    ...
    console.error('Erreur lors du chargement des tutoriels achetés:', err)
  }
}
...
const purchaseTutorial = async () => {
  try {
    ...
    const response = await api.post('/user/tutorials/purchase', {
