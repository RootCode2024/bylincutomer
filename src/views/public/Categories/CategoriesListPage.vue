...
const totalSubcategories = computed(() => {
  return categories.value.reduce((total, category) => total + (category.subcategories_count || 0), 0)
})
...
const getImageUrl = (iconPath) => {
  ...
  if (iconPath.startsWith('/')) {
    return `${import.meta.env.VITE_API_BASE_URL}${iconPath}`
  }
  
  // Sinon, on suppose que c'est un chemin relatif depuis le dossier des images
  return `${import.meta.env.VITE_API_BASE_URL}/storage/${iconPath}`
}
