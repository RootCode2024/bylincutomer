<template>
  <div class="min-h-screen bg-white">
    ...
    <section
      class="relative bg-gradient-to-br from-indigo-700 via-indigo-700 to-indigo-800 text-white overflow-hidden min-h-screen flex items-center">
      ...
      <div class="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div class="absolute bottom-1/3 right-1/4 w-96 h-96 bg-white/10 rounded-full blur-3xl animate-float-delayed">
        </div>
        <div class="absolute top-1/2 right-1/3 w-64 h-64 bg-indigo-400/20 rounded-full blur-3xl animate-float-slow">
        </div>
      </div>
      ...
      <div class="relative container mx-auto px-6 py-20">
        <div class="grid lg:grid-cols-2 gap-16 items-center max-w-7xl mx-auto">
          ...
          <div class="space-y-8 animate-fade-in-left">
            <!-- Elegant badge -->
            <div
              class="inline-flex items-center gap-2 bg-white/5 backdrop-blur-md border border-white/20 rounded-full px-4 py-2 text-xs font-medium tracking-widest">
          ...
          <div class="relative animate-fade-in-right">
            <div class="relative group">
              <!-- Decorative frame -->
              <div
                class="absolute -inset-4 bg-gradient-to-br from-indigo-500/20 to-white/10 rounded-3xl blur-xl group-hover:blur-2xl transition-all duration-500">
              </div>

              <!-- Image container -->
              <div class="relative rounded-2xl overflow-hidden border-2 border-white/20 shadow-2xl">
                <img src="https://upload.wikimedia.org/wikipedia/commons/2/2d/Behanzin-1895.jpg" alt="Roi Béhanzin"
              ...
              <div
                class="absolute top-0 left-0 w-12 h-12 border-t-2 border-l-2 border-white/30 rounded-tl-2xl -translate-x-2 -translate-y-2">
              </div>
              <div
                class="absolute bottom-0 right-0 w-12 h-12 border-b-2 border-r-2 border-white/30 rounded-br-2xl translate-x-2 translate-y-2">
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="container mx-auto px-6 py-4">
        <div class="flex flex-col lg:flex-row items-stretch lg:items-center gap-3">
          ...
          <div class="flex-1 max-w-md">
            <div class="relative group">
              <Search
                class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-black/40 group-focus-within:text-indigo-800 transition-colors" />
        ...
        <transition name="slide-down">
          <div v-if="showFilters" class="mt-4 p-4 bg-black/5 rounded-xl border border-black/10">
            <label class="flex items-center gap-2 cursor-pointer group w-fit">
              <input v-model="filters.featured" type="checkbox" @change="handleFilterChange"
    ...
    <section class="py-16 bg-white">
      <div class="container mx-auto px-6">
        ...
        <div v-else-if="collections.length === 0" class="text-center py-20">
          ...
          <button @click="searchQuery = ''; filters.featured = false; handleFilterChange()"
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div v-for="(collection, index) in collections" :key="collection.id"
            class="group cursor-pointer animate-fade-in-up" :style="{ animationDelay: `${index * 50}ms` }">
            <div
              class="relative overflow-hidden rounded-2xl bg-white border border-black/5 transition-all duration-500 hover:border-indigo-800/30 hover:shadow-xl hover:shadow-indigo-800/5 hover:-translate-y-1">
              <!-- Image -->
              <div class="relative aspect-[3/4] overflow-hidden bg-black/5">
                <img :src="getFullImageUrl(collection)" :alt="collection.name"
                ...
                <div class="absolute top-3 left-3 flex flex-col gap-1.5">
                  <div v-if="collection.is_featured"
    ...
    <section
      ...
      <div class="relative container mx-auto px-6 text-center">
        <div class="max-w-2xl mx-auto space-y-6">
          <div
            class="inline-flex items-center gap-2 bg-white/5 border border-white/20 rounded-full px-3 py-1.5 text-xs font-medium mb-4">
            <Sparkles class="w-3 h-3" />
          ...
          <div class="flex flex-col sm:flex-row items-center justify-center gap-3 pt-6">
            <router-link to="/contact-us"
              class="px-8 py-3 bg-transparent border border-white text-white rounded-xl font-semibold text-sm hover:bg-white hover:text-indigo-800 transition-all">
              Nous contacter
            </router-link>
          </div>
        </div>
...
const fetchCollections = async (page = 1) => {
  try {
    ...
    const response = await productStore.fetchCollections({ params })
    console.log('Collection :', productStore.allCollections)
    if (response) {
      collections.value = productStore.collections.data || []
    } else {
      collections.value = []
    }
  } catch (error) {
